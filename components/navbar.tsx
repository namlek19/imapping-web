...
export default function Navbar() {
  ...
  const searchInputJSX = (ref: React.RefObject<HTMLInputElement | null>) => (
  ...
  return (
    <div className="sticky top-0 z-50 px-4 pt-5 pb-2 pointer-events-none">
      <nav className={`max-w-6xl mx-auto bg-surface/80 backdrop-blur-lg border border-slate-200/50 shadow-sm shadow-slate-200/50 pointer-events-auto transition-all duration-200 ${mobileMenuOpen ? "rounded-3xl" : "rounded-full"}`}>

        {/* ── Main row ── */}
        <div className="px-4 md:px-6 py-3 flex items-center justify-between gap-3 md:gap-6">

          {/* Logo */}
          <Link href="/" className="select-none shrink-0">
            <span className={`${spaceGrotesk.className} text-xl md:text-2xl font-bold text-accent tracking-tight`}>
              iMapping
            </span>
          </Link>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {isLoggedIn ? (
              ...
              <Link
                href="/login"
                className="px-4 md:px-6 py-2 md:py-2.5 rounded-full bg-accent text-white text-sm font-bold shadow-sm shadow-accent/30 hover:shadow-md hover:shadow-accent/30 active:scale-95 transition-all duration-200"
              >
                Đăng nhập
              </Link>
            )}
            ...
            <button
              className="md:hidden w-9 h-9 rounded-full flex items-center justify-center text-primary hover:bg-gray-100 transition-colors"
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-slate-200/50 px-4 pb-4 pt-3 flex flex-col gap-3">
            {/* Search */}
            <div ref={mobileSearchRef} className="relative">
              {searchInputJSX(mobileInputRef)}
              {searchResultsJSX}
            </div>
              ...
              {NAV_LINKS.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center px-4 py-3 rounded-xl text-sm font-bold transition-all duration-150 ${
                      isActive
                        ? "bg-accent/10 text-accent"
                        : "text-primary/70 hover:text-primary hover:bg-gray-100"
                    }`}
